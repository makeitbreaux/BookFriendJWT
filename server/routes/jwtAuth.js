...
router.post("/register", validInfo, async (req, res) => {
    ...
    try {
        ...
        let newUser = await pool.query(`INSERT INTO users (user_first_name, user_last_name, user_email, user_password) VALUES ($1, $2, $3, $4)`, [user_first_name, user_last_name, user_email, bcryptPassword]);
        
        //5. GENERATING OUR JWT TOKEN
        const token = jwtGenerator(newUser.rows[0].user_id);
        return res.json({token}), res.send({newUser});
...
router.post("/login", validInfo, async (req, res) => {
    ...
    try {
        ...
        return res.json({token}), res.send({user});;
